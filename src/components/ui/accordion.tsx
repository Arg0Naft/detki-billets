...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn("border-b border-slate-200/80 last:border-b-0", className)}
    {...props}
  />
));
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Header className="flex">
    <AccordionPrimitive.Trigger
      ref={ref}
      className={cn(
        "flex flex-1 items-center justify-between gap-4 py-5 text-left text-base font-semibold text-slate-900 cursor-pointer transition-colors [&[data-state=open]>svg]:rotate-180",
        className,
      )}
      {...props}
    >
